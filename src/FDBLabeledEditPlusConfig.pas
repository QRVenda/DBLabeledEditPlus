{==============================================================================]
[ Unit           : FDBLabeledEditPlusConfig.pas                                ]
[ Componente     : "TDBLabeledEditPlus"                                        ]
[ Escopo         : Tela de configuração de script SQL e definições de campos.  ]
...
begin
  ...
  for oFieldType := Low(TFieldType) to High(TFieldType) do
    cbxFieldType.Items.AddObject(GetEnumName(TypeInfo(TFieldType), Ord(oFieldType)), TObject(oFieldType));
  cbxDefaultComparison.Items.Clear;
  for oSearchComparison := Low(TSearchComparison) to High(TSearchComparison) do
    cbxDefaultComparison.Items.AddObject(GetEnumName(TypeInfo(TSearchComparison), Ord(oSearchComparison)), TObject(oSearchComparison));
  cbxSearchCase.Items.Clear;
  for oSearchCase := Low(TSearchCase) to High(TSearchCase) do
    cbxSearchCase.Items.AddObject(GetEnumName(TypeInfo(TSearchCase), Ord(oSearchCase)), TObject(oSearchCase));
  ClearProperties();
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        oEditDialogField := TEditDialogField(lbxFields.Items.Objects[i]);
        oEditDialogField.FieldName := edtFieldName.Text;
...
begin
  ...
  begin
    ...
    begin
      oEditDialogField := TEditDialogField(lbxFields.Items.Objects[i]);
      oEditDialogField.WhereSyntax := edtWhereSyntax.Text;
    end;
  end;
end;
