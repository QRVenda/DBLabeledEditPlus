...
begin
  ...
  FLabelClicar.Hint    := '';
  cTecla               := Trim(ShortCutToText(FConfigurar.FTeclaPesquisa));
  if Trim(FConfigurar.RecursoExtra.BotaoCaption) <> '' then
    cTecla := Trim(FConfigurar.RecursoExtra.BotaoCaption);
  FLabelClicar.Caption  := '';
  FLabelClicar.Hint     := '';
  if cTecla <> '' then
  begin
    FLabelClicar.Caption  := cTecla;
    FLabelClicar.Hint     := 'Tecle ' + cTecla + ' para pesquisar';
  end;
end;
...
begin
  ...
  if cTecla <> '' then
  begin
    FBotaoClicar.Caption  := cTecla;
    FBotaoClicar.Hint     := 'Tecle ' + cTecla + ' para pesquisar';
  end;
end;
...
begin
  ...
  LimparTexto := (csDesigning in ComponentState) and (Text = '');
...
begin
  ...
  nTopo     := FEditButtonExtra.Top - 1;
  nAltura   := FEditButtonExtra.Height + 2;
  nLargura  := FEditButtonExtra.Width;

  case FRecursoExtra.BotaoTipo of
    trNenhum  : nLargura := nLargura + 3;
    trLabel   : nLargura := nLargura + FEditButtonExtra.LabelClicar.Width;
    trBotao   : nLargura := nLargura + FEditButtonExtra.BotaoClicar.Width;
  end;
  nEsquerda  := FEditButtonExtra.Left + nLargura;

  if (FRecursoExtra.LocalPosicao = lpAbaixo)  then
  begin
    nTopo      := FEditButtonExtra.Top + FEditButtonExtra.Height + 3;
    nEsquerda  := FEditButtonExtra.Left;
  end;

  case FRecursoExtra.LabelResultado of
    lmLabel : begin
                FEditButtonExtra.LabelMostrar.Height := nAltura;
                FEditButtonExtra.LabelMostrar.SetBounds(nEsquerda, nTopo, FEditButtonExtra.LabelMostrar.Width, FEditButtonExtra.LabelMostrar.Height);
              end;
...
begin
  ...
  try
    ...
    Confirmado := FrmDBLabeledEditPlusPesquisar.ShowModal= mrOk;
...
begin
  ...
  if FConfigurar.EventoPesquisa = epNaoPesquisar then
    Exit;
  FConfigurar.Resultado := '';
  cScript := Trim(FConfigurar.SQL.Text);
  if cScript = '' then
    exit;
  FConfigurar.RegistroExiste := False;
  for i := 0 to FConfigurar.Campos.Count - 1 do
  begin
    oCampo := TEditDialogField(FConfigurar.Campos[i]);
    if oCampo.FieldName = FConfigurar.CampoChave then
    begin
      oFieldType := oCampo.FieldType;
      Break;
    end;
  end;

  cWhere := '';
  if (oFieldType in [ftSmallint, ftInteger, ftWord, ftAutoInc, ftLargeint, ftByte, ftBytes, ftLongWord]) then
  begin
    cWhere := FConfigurar.CampoChave + ' = ' + StringReplace(aValue, '.', '', [rfReplaceAll]);
  end
  else
  begin
    cWhere := FConfigurar.CampoChave + ' = ' + QuotedStr(aValue)
  end;

  cScript := StringReplace(cScript, WHERE_TAG, cWhere, [rfIgnoreCase]);
  try
    FConfigurar.Connection.ExecSQL(cScript, nil, oDataSet);
    ...
    FConfigurar.Resultado := Trim(oDataSet.FieldByName(FConfigurar.CampoResultado).AsString);
...
begin
  ...
  begin
    ...
    begin
      oRegistro.FieldName  := aDataSet.Fields[i].FieldName;
      oRegistro.FieldValue := aDataSet.Fields[i].AsString;
      oRegistro.IsNull     := aDataSet.Fields[i].IsNull;
      Result[i]            := oRegistro;
    end;
  end;
end;
